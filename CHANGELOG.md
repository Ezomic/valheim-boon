...
- Every character level earns one **pick**, spent on any card in the catalogue to raise it a
  rank, up to five.
- **Picks bank.** Nothing expires and the panel waits.
- **No drawbacks.** A card is a reward or it is nothing.

### Free choice, not a dealt hand

An earlier version dealt three cards per level, seeded so that quitting on a bad offer and
...
What it costs is the tension of a hand you are dealt. A free choice means the strongest card
is always available, so balance now lives in the cards themselves and in `MaxRank`.

The panel carries the weight for this: choosing between fourteen cards is only a real choice
if each tile says both what it is worth now and what the pick would make it.

### Capacity is a card, not a setting
...
- Card balance is untested by definition: with free choice, the strongest card is always
